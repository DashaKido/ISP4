...
class Cache(redis.StrictRedis):
    def __init__(self, host, port, password, charset="utf-8", decode_responses=True):
        super(Cache, self).__init__(host, port, password=password, charset=charset, decode_responses=decode_responses)
...
class Database:
    ...
    async def insert_users(self, user_id: int, deadlines: str):
        insert_query = f"""INSERT INTO users (id, deadlines)
                           VALUES ({user_id}, "{deadlines}")"""
        self._execute_query(insert_query)
        logging.info(f"Deadlines for user {user_id} added")

    async def select_users(self, user_id: int):
        select_query = f"""SELECT deadlines from users 
                           where id = {user_id}"""
        record = self._execute_query(select_query, select=True)
        return record

    async def update_users(self, user_id: int, deadlines: str):
        update_query = f"""Update users 
                              set deadlines = "{deadlines}" where id = {user_id}"""
        self._execute_query(update_query)
        logging.info(f"Deadlines for user {user_id} updated")
    ...
    async def insert_or_update_users(self, user_id: int, deadlines: str):
        user_deadlines = await self.select_users(user_id)
        if user_deadlines is not None:
            await self.update_users(user_id, deadlines)
        else:
            await self.insert_users(user_id, deadlines)
